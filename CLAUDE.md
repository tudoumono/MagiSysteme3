...
### 段階的実装アプローチ（Phase 1の例）

各フェーズは以下の順序で段階的に実装する：

```
Phase 1: 判定モード
├── Step 1: 同期版（まず動くものを作る）
│   └── judge_mode() → 3エージェント実行 → JUDGE統合 → FinalVerdict返却
├── Step 2: ストリーミング版
│   └── stream_async() でリアルタイムイベント取得
└── Step 3: 思考・ツール使用表示
    └── thinking, tool_use, reasoning イベント処理
```
...
```
+------------------+     +------------------+     +------------------+
|   Step 1: 同期版  | --> |  Step 2: Stream  | --> | Step 3: Events  |
+------------------+     +------------------+     +------------------+
        |                        |                        |
   judge_mode()            stream_async()           イベント処理
   (B1.5)                  (B1.6)                   (B1.7)
```
...
再学習・ハンズオン形式で繰り返し学べるドキュメント構成：

```
docs/
├── 02_current_progress.md        # 進捗管理（軽量、現在地の確認用）
├── 03_code_snapshot.md           # 動作確認済みコードのスナップショット
├── handson/                      # ハンズオン教材（Step別の実装手順）
└── reference/                    # 参照資料（SDK仕様・トラブルシューティング）
```

> 詳細なドキュメント更新ルール・フォーマットは `.claude/skills/doc-update.md` を参照

## Skills（作業ルール）

作業の進め方やルールは `.claude/skills/` に定義：

| Skill | 用途 | トリガー |
|-------|------|---------|
| `doc-update.md` | ドキュメント更新ルール | Step完了時 |
| `pair-programming.md` | ペアプロ形式の進め方 | 実装開始時 |
| `code-review.md` | コードレビュー観点 | 実装完了時 |
